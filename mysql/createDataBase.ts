...
const connection = mysql.createConnection({
    host: mysqlConfig.host, // 数据库服务器地址
...
async function handleFun() {
    // 1. 创建数据库
    let data: { success: boolean, data: any } = await doSql(`create database if not exists blog_data`, connection);
    if (!data.success) {
    ...
    // 2. 使用 blog_data 数据库
    data = await doSql(`use blog_data`, connection);
    if (!data.success) {
        console.log(data.data.message);
        connection.destroy(); //  关闭链接
        return;
    ...
    // 3.  创建users数据库表
    data = await doSql(`create table if not exists users(
    ...
    // 插入一个默认用户
    data = await doSql(`insert into users(email,password,nick) values('hzy@gmail.com','test1','123')`, connection);
    if (!data.success) {
        console.log("user:" + data.data.message);
        connection.destroy(); //  关闭链接
        return;
    ...
    // 4.  创建博客数据库表
    data = await doSql(`create table if not exists blogs(
    id bigint primary key auto_increment,
    uid int not null,
    ...
    // 5.  创建评论数据库表
    data = await doSql(`create table if not exists articles(
    id bigint primary key auto_increment,
    uid int not null,
